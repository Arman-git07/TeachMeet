...
import { Clock, User, Pencil, Save, Loader2 } from 'lucide-react';
...
export function SubjectTeachers() {
    ...
    const handleUpdateTeacher = async () => {
        ...
        try {
            ...
            toast({ variant: 'destructive', title: "Update Failed", description: "You might not have permission to edit this." });
        } finally {
            setIsSaving(false);
        }
    };
                                            ...
                                            <div className="bg-muted/30 rounded-lg p-3 border border-border/50 relative">
                                                ...
                                                {isCreator && (
                                                    <Button 
                                                        variant="ghost" 
                                                        size="icon" 
                                                        className="absolute top-2 right-2 h-7 w-7 opacity-0 group-hover:opacity-100 transition-opacity rounded-full bg-background/80 shadow-sm"
