...
export default function MeetingChatPage({ params }: { params: { meetingId: string } }) {
  ...
  useEffect(() => {
    setMessages([{ id: 'welcome', senderName: 'System', text: `Welcome to the chat for ${topic}. Messages are not persisted.`, timestamp: new Date(), isMe: false }]);
  }, [topic]);
    ...
    <div className="flex flex-col h-full bg-muted/30">
      ...
      <main className="flex-grow flex flex-col overflow-hidden">
       ...
       <footer className="flex-none p-2 text-center text-xs text-muted-foreground border-t bg-background">
        Chat is for demonstration purposes only. Messages are not sent.
      </footer>
    </div>
  );
}
