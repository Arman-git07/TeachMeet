...
const DocumentRow = ({ doc, onDelete, currentUserId }: { doc: Document; onDelete: (id: string, name: string, storagePath: string) => void; currentUserId: string | undefined }) => {
  ...
  const renderDocumentList = (docs: Document[], emptyState: React.ReactNode) => {
    ...
    return (
      <div className="space-y-2">
        {docs.map(doc => <DocumentRow key={doc.id} doc={doc} onDelete={handleOpenDeleteDialog} currentUserId={currentUser?.uid} />)}
      </div>
    );
  };
