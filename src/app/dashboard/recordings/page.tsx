...
const RecordingCard = ({ rec, onDelete, currentUserId }: { rec: Recording; onDelete: (id: string, name: string, storagePath: string) => void; currentUserId: string | undefined }) => {
...
export default function RecordingsPage() {
  ...
  const renderRecordingGrid = (recs: Recording[], emptyState: React.ReactNode) => {
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {recs.map(rec => <RecordingCard key={rec.id} rec={rec} onDelete={handleOpenDeleteDialog} currentUserId={currentUser?.uid} />)}
      </div>
    );
  };
