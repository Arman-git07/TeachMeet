...
export default function TeachingPage() {
  ...
  useEffect(() => {
    ...
    setHeaderContent(
      <div className="min-w-0 flex-1">
        ...
        <p className="text-sm text-muted-foreground mt-1 truncate">Taught by: {teaching.teacherName}</p>
      </div>
    );
    setHeaderAction(
  ...
  }, [teaching, setHeaderContent, setHeaderAction]);
  ...
  useEffect(() => {
    ...
    const unsubTeaching = onSnapshot(doc(db, 'teachings', teachingId), (docSnap) => {
      if (docSnap.exists()) {
        ...
        toast({ variant: 'destructive', title: 'Error', description: 'Teaching not found.' });
  ...
  const handleRequestAction = useCallback(async (requestId: string, approve: boolean) => {
    ...
    try {
        ...
        const requestRef = doc(db, 'teachings', teachingId, 'joinRequests', requestId);
        ...
        if (approve) {
            ...
            const enrollmentRef = doc(db, 'users', studentData.studentId, 'enrolled', teachingId);
        ...
        toast({ title: `Request ${approve ? 'Approved' : 'Denied'}`, description: `The student has been ${approve ? 'added to the teaching' : 'denied entry'}.` });
    } catch (error) {
        console.error("Error handling join request:", error);
        toast({ variant: 'destructive', title: 'Error', description: 'Could not process the request. Check Firestore rules.' });
    }
  }, [isTeacher, teachingId, teaching?.title, teaching?.description, teaching?.teacherName, toast]);
  ...
  const handlePostAnnouncement = async () => {
    ...
    try {
      ...
      if (audioAttachment) {
        const audioRef = storageRef(storage, `teachings/${teachingId}/announcements/${Date.now()}.webm`);
        const snapshot = await uploadBytes(audioRef, audioAttachment);
        audioURL = await getDownloadURL(snapshot.ref);
      }

      await addDoc(collection(db, `teachings/${teachingId}/announcements`), {
  ...
  if (!teaching) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center p-4 md:p-8">
        ...
        <h2 className="text-2xl font-bold">Teaching Not Found</h2>
        <p className="text-muted-foreground">The teaching you are looking for does not exist or has been deleted.</p>
        <Button asChild className="mt-6">
          <Link href="/dashboard/teachings">Go Back to Teachings</Link>
        </Button>
      </div>
    );
  }
              ...
              <TabsContent value="subjects" className="mt-0">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Overview of subjects covered in this teaching.</CardDescription>
                    </CardHeader>
                    <CardContent className="text-center text-muted-foreground py-12">
